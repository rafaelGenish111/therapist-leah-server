...
router.post('/', async (req, res) => {
    try {
        ...
        res.status(201).json({
            message: 'Health declaration send successfully',
        ...
        if (error.name === 'validationError') {
            ...
            return res.status(400).json({ messages: messages.join(', ') });
        }
    ...
    try {
        ...
        if (req.query.fromDate || req.query.toDate) {
            ...
            if (req.query.fromDate) {
                query.createAt.$gte = new Date(req.query.fromDate);
            }
            if (req.query.toDate) {
                query.createAt.$lte = new Date(req.query.toDate);
            }
        }
...
router.delete('/:id', authenticateToken, async (req, res) => {
    try {
        ...
        console.error('Deleted health declaration error:', error);
